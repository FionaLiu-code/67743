...
let filtered = humanOfficers.filter { officer in
    return femaleOfficers.contains(officer)
} //
print(Float(filtered.count) / Float(humanOfficers.count))


// Q2: How many first officers are human?

let humanFirst = firstOfficers.keys.filter { return humanOfficers.contains($0) }
...
let array = captains.map{ Officer( name: $0, ship: $1) }
...
let arrayQ6 = firstOfficers.keys.filter { return vulcanOfficers.contains($0) }
print(arrayQ6.count)
...
let arrayQ7 = arrayQ6.filter { return $0.count < 5 }
print(arrayQ7.count)
